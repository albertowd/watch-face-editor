    ...
    <v-card-title>
      <v-layout>
        ...
        <input
          ref="numberInput"
          accept=".png"
          class="editor-activity-distance-number-hidden"
          multiple
          type="file"
          @change="numberFilePicked"
        />
        <v-btn class="ml-5" @click="decimalFilePick">
          <h2 :class="decimalImage ? 'primary--text' : 'normal--text'">{{ tDecimal }}</h2>
        </v-btn>
        <input
          ref="decimalInput"
          accept=".png"
          class="editor-activity-distance-number-hidden"
          type="file"
          @change="decimalFilePicked"
        />
        <v-btn class="ml-5" @click="kmFilePick">
          <h2 :class="kmImage ? 'primary--text' : 'normal--text'">km</h2>
        </v-btn>
        <input
          ref="kmInput"
          accept=".png"
